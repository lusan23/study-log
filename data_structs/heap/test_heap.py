...
class TestHeap(unittest.TestCase):
    ...
    def test_levels(self):
        hp = Heap()
        hp.root.update_data(50)

        hp.root.insert_left(50)
        hp.root.insert_right(60)

        hp.root.left.insert_left(30)        
        hp.root.left.insert_right(70) 
        hp.root.right.insert_left(80)
        hp.root.right.insert_right(90)

        level = hp.find_level(hp.root, 30)
        levels = hp.find_levels(level)
        print(f"the level of {1} is {level}{levels}")
